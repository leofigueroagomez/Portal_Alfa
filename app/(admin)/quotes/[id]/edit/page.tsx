...
export default function EditQuotePage() {
  ...
  useEffect(() => {
    async function loadQuote() {
      ...
      if (quoteData.indirect_cost_percent != null) {
        setCompanyIndirectCostPercent(Number(quoteData.indirect_cost_percent));
      } else {
        ...
        setCompanyIndirectCostPercent(
          Number(settingsData?.indirect_cost_percent || 0)
        );
      }

      const { data: miscItemsData } = await supabase
        .from("quote_misc_items")
        .select("id, description, amount_mxn")
        .eq("quote_id", quoteId)
        .order("sort_order", { ascending: true });

      setMiscItems(
        (miscItemsData || []).map((item) => ({
          id: String(item.id),
          description: item.description || "",
          amount: String(item.amount_mxn || ""),
        }))
      );
      ...
      const [{ data: categoryData }, { data: tagData }, { data: laborCatalogData }, { data: systemsData }] = await Promise.all([
        supabase
      ...
      setQuotableSystems(systemsData || []);
      ...
      let { data: itemsData, error: itemsError } = await supabase
        .from("quote_items")
        .select(
          "id, quote_section_id, product_id, quantity, sale_currency, unit_equipment_price, unit_equipment_price_usd, unit_labor_price, equipment_total_usd, product_brand, product_model, product_name, product_image_url, existing_customer_equipment, area, customer_visible_note, sort_order"
        )
        .eq("quote_id", quoteId)
        .order("sort_order", { ascending: true });
      ...
      const { data: savedAreaAllocations, error: areaAllocationsError } =
      ...
      if (!areaAllocationsError) {
        ((savedAreaAllocations || []) as SavedAreaAllocation[]).forEach(
          (allocation) => {
            const quoteItemId = Number(allocation.quote_item_id);
            const current = areaAllocationsByItemId.get(quoteItemId) || [];
            current.push({
              id: allocation.id,
            ...
            areaAllocationsByItemId.set(quoteItemId, current);
          }
        );
      }
  ...
  async function handleSaveQuote() {
    ...
    if (
      ...
      (updateResult.error.message.includes("client_id") ||
        ...
        updateResult.error.message.includes("indirect_cost_percent") ||
        updateResult.error.message.includes("indirect_cost_mxn") ||
        updateResult.error.message.includes("misc_total_mxn") ||
    ...
    for (const [sectionIndex, section] of sections.entries()) {
      ...
      if (areaAllocationsToInsert.length > 0) {
        ...
        if (areaAllocationsError) {
          reportStepError("crear distribucion por area", areaAllocationsError);
          setSavingQuote(false);
          return;
        }
      }
    ...
    const { error: deleteMiscItemsError } = await supabase
      .from("quote_misc_items")
      .delete()
      .eq("quote_id", quoteId);

    if (deleteMiscItemsError) {
      reportStepError("borrar quote_misc_items", deleteMiscItemsError);
      setSavingQuote(false);
      return;
    }
    ...
    if (miscItemsToInsert.length > 0) {
      ...
      if (insertMiscItemsError) {
        reportStepError("guardar quote_misc_items", insertMiscItemsError);
        setSavingQuote(false);
        return;
      }
    }
        ...
        <div className="space-y-8 xl:col-span-2">
          ...
          <div className="rounded-2xl border border-[#1F1F24] bg-[#151518] p-4 sm:p-6">
            <div className="mb-6 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              ...
              <div className="flex flex-col gap-3 sm:flex-row items-stretch sm:items-center xl:justify-end">
                <select
                          ...
                          <div className="mt-4 grid gap-3 border-t border-[#2A2A30] pt-4 md:grid-cols-[minmax(0,1fr)_220px]">
                            <div className="space-y-3">
                              ...
                              <input
                                ...
                                className="w-full rounded-xl border border-[#2A2A30] bg-[#151518] px-4 py-3 text-sm outline-none focus:border-[#9E1B32] disabled:cursor-not-allowed disabled:opacity-70"
                              />
