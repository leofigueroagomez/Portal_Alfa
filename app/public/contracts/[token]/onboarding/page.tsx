...
import { getContractByOnboardingToken } from "./actions";
import ClientContractOnboardingForm from "./ClientContractOnboardingForm";
...
export async function generateMetadata({
  ...
  try {
    const contract = await getContractByOnboardingToken(token);
    const projectName = (contract.client_projects as { name?: string } | null)?.name || "Proyecto";
    return {
      title: `Onboarding de Contrato - ${contract.contract_number} (${projectName}) | ALFA IT`,
      description: `Formalización contractual para el proyecto de integración tecnológica con ALFA IT.`,
    };
  } catch {
    return {
      title: "Onboarding de Contrato | ALFA IT",
    };
  }
}
...
export default async function ContractOnboardingPage({
  ...
  try {
    ...
    return <ClientContractOnboardingForm token={token} contract={contract} />;
  } catch (error) {
    const errorMessage =
