...
| Detalle de servicio | `app/portal/services/[id]/page.tsx`, `lib/clientPortal.ts`, `lib/contractorPortal.ts`, `app/portal/services/[id]/ContractorServiceView.tsx` | `service_reports.client_id`, `service_reports.contractor_id`, `service_report_photos.image_url` | Alto: el codigo filtra por cliente/proyecto o contratista asignado; subcontratistas pueden capturar bitacora y fotos. |
| Portal de subcontratistas | `app/portal/page.tsx`, `lib/contractorPortal.ts`, `app/portal/services/[id]/ContractorServiceView.tsx` | `contractor_portal_users`, `service_reports.contractor_id`, `work_orders.contractor_id` | Critico: aislamiento total; solo ven servicios y ordenes de su empresa, sin acceso a precios de venta ni datos financieros. |
| Administrar usuarios portal cliente | `app/(admin)/clients/[id]/portal-users/page.tsx`, `app/(admin)/clients/[id]/portal-users/actions.ts` | `client_portal_users`, `client_portal_project_access`, Supabase Auth admin invite | Critico: solo `admin` puede crear, invitar, desactivar o asignar proyectos. |
| Administrar usuarios subcontratista | `app/(admin)/contractors/[id]/portal-users/page.tsx`, `actions.ts`, `ContractorPortalUsersClient.tsx` | `contractor_portal_users`, Supabase Auth admin `generateLink`, invitaciones directas por WhatsApp | Critico: solo `admin`; genera tokens unicos y mensajes de WhatsApp listos para enviar. |
