# Module Index

Indice operativo para ubicar rapido donde trabajar. Los niveles de riesgo se basan en impacto potencial observado en rutas, auth, fiscalidad, PDF, datos y produccion.

| Modulo | Estado Inferido | Rutas Principales | Archivos/Carpetas Clave | Doc | Riesgo |
| --- | --- | --- | --- | --- | --- |
| Cotizaciones | Activo, critico para ventas y PDF Premium | `app/(admin)/quotes/`, `app/api/quotes/` | `lib/quotePremiumPdf.ts`, `lib/quotePremiumPdfHtml.ts`, `lib/quotePdfSnapshot.ts`, `lib/quoteDiagnosticContext.ts`, `sql/*quote*` | [`../modules/cotizaciones/MODULE_CONTEXT.md`](../modules/cotizaciones/MODULE_CONTEXT.md) | Critico |
| Facturacion | Activo, fiscal | `app/(admin)/invoices/`, `app/api/invoices/`, `app/api/payment-complements/` | `lib/facturama.ts`, `lib/invoices.ts`, `lib/invoiceProration.ts`, `lib/paymentComplements.ts`, `lib/satBillingProviders.ts` | [`../modules/facturacion/MODULE_CONTEXT.md`](../modules/facturacion/MODULE_CONTEXT.md) | Critico |
| Portal cliente | Activo, separacion de datos cliente/admin | `app/portal/`, `app/(client)/`, `app/public/documents/` | `lib/clientPortal.ts`, `lib/publicDocuments.ts`, `proxy.ts`, `lib/apiAuth.ts` | [`../modules/portal-cliente/MODULE_CONTEXT.md`](../modules/portal-cliente/MODULE_CONTEXT.md) | Critico |
| Proyectos | Activo | `app/(admin)/projects/` | `lib/project*`, `app/api/projects/` | [`../modules/proyectos/MODULE_CONTEXT.md`](../modules/proyectos/MODULE_CONTEXT.md) | Alto |
| Servicios | Activo | `app/(admin)/services/`, `app/portal/services/` | `app/api/services/`, librerias `service*` si existen | [`../modules/servicios-postventa/MODULE_CONTEXT.md`](../modules/servicios-postventa/MODULE_CONTEXT.md) | Alto |
| Compras y contratistas | Activo | `app/(admin)/contractors/`, rutas de compras bajo proyectos | `lib/projectOperationalItems.ts`, `app/api/projects/` | Pendiente de crear | Alto |
| Productos y catalogo admin | Activo | `app/(admin)/products/`, `product-categories`, `product-tags` | `app/(admin)/products/`, storage de imagenes si aplica | Pendiente de crear | Medio |
| Sitio publico (SEO tecnico e indexacion) | Activo. Dominio canonico fijado en codigo + 301 heredadas (2026-09-02) | `app/robots.ts`, `app/sitemap.ts`, `app/layout.tsx` | `lib/siteUrl.ts`, `next.config.ts` (`legacyRedirects`) | [`../modules/sitio-publico/MODULE_CONTEXT.md`](../modules/sitio-publico/MODULE_CONTEXT.md) | Alto |
| Marcas & Catalogo Publico | Activo, critico para SEO y Leads | `app/marcas/`, `app/marcas/[marca]/`, `app/marcas/[marca]/[modelo]/` | `lib/catalog.ts`, `lib/catalogData.ts`, `lib/catalogBrandUi.ts`, `public/products/`, `sql/*radiora3*`, `sql/*sonos*` | [`../modules/catalogo-marcas/MODULE_CONTEXT.md`](../modules/catalogo-marcas/MODULE_CONTEXT.md) | Medio |
| Portafolio & Casos de Estudio | Activo, credibilidad y SEO | `app/portafolio/`, `app/portafolio/[slug]/` | `lib/portfolio.ts`, `lib/portfolioData.ts`, `public/portfolio/` | [`../modules/portafolio/MODULE_CONTEXT.md`](../modules/portafolio/MODULE_CONTEXT.md) | Medio |
| Aliados comerciales | Activo | `app/(admin)/commercial-partners/` | `lib/commercialPartners.ts`, SQL de white-label | Pendiente de crear | Alto |
| Leads y landing publica | Activo | `app/page.tsx`, `app/(admin)/leads/` | `app/public/`, docs de auditoria si aplica | Pendiente de crear | Medio |
| Usuarios y configuracion | Activo | `app/(admin)/users/`, `app/(admin)/settings/` | `services/profile.ts`, `lib/permissions.ts`, `proxy.ts` | [`../modules/usuarios-configuracion/MODULE_CONTEXT.md`](../modules/usuarios-configuracion/MODULE_CONTEXT.md) | Critico |
| Dashboard direccion | Activo | `app/(admin)/director-dashboard/`, `app/(admin)/dashboard/` | reportes y SQL relacionados | Pendiente de crear | Alto |
| Ingenieria | Activo | `app/(admin)/engineering/`, `app/(admin)/engineering-quotes/` | rutas bajo `app/(admin)/engineering*` | Pendiente de crear | Medio |
| Notificaciones | Activo | `app/(admin)/notifications/` | rutas/API relacionadas a notificaciones | Pendiente de crear | Medio |
| Postventa | Activo | `app/(admin)/post-sale/` | rutas bajo `post-sale` | [`../modules/servicios-postventa/MODULE_CONTEXT.md`](../modules/servicios-postventa/MODULE_CONTEXT.md) | Alto |
| El Vigia (capa de vigilancia autonoma) | Activo desde 2026-08-31. Fase 1 (15 sensores) + Bandeja `/vigia` + ejecutores 1 clic (B1) + Investigar a fondo (B2) | `app/api/vigia/cron/daily/`, `app/(admin)/vigia/` | `lib/vigia/*` (incl. `executors.ts`, `investigate/*`), tablas `vigia_*`, vistas `vigia_v_*`, `sql/2026090*_vigia_*`, `vercel.json` (cron) | [`../modules/vigia/MODULE_CONTEXT.md`](../modules/vigia/MODULE_CONTEXT.md) | Medio |

## Uso Para Agentes

1. Ubicar el modulo en esta tabla.
2. Leer su `MODULE_CONTEXT.md` si existe.
3. Verificar rutas con `rg --files`.
4. Revisar `SECURITY_RULES.md` antes de tocar auth, storage, SQL o API routes.

## Pendientes De Fase 2

- Crear contexto especifico para compras/contratistas y aliados comerciales.
- Confirmar tablas exactas por modulo desde Supabase o migraciones completas.
- Documentar comandos de validacion por modulo.
